"""KD-tree and Ball-tree kNN queries with skip-ball and per-index k expansion.

``query_init`` is the parallel all-points kNN.
``query_extend(indices, skip_radii, knn_scope)`` queries a subset: neighbors
inside the skip ball are counted in ``knn_skip`` and omitted. ``knn_scope`` is
the per-index neighbor count (not a communal scalar K). ``dist`` / ``ind`` have
width at least ``max(knn_scope)`` and grow to include every neighbor tied at
the k-th distance. Unused slots are ``0``, not ``INF``.
...
def _out_from_rdist(rd, metric_id, p, angular_mode):
    """Tree rdist -> original metric, matching query_init conversion."""
    if angular_mode == MET_COSINE:
        d = math.sqrt(rd)
        return 0.5 * d * d
    if angular_mode == MET_ARCCOS:
        d = math.sqrt(rd)
        x = 1.0 - 0.5 * d * d
        if x < -1.0:
            x = -1.0
            ...
            x = 1.0
        return math.acos(x)
    return _rdist_to_dist(rd, metric_id, p)
...
@njit(cache=True)
def _kd_max_rdist(bounds, i_node, pt, metric_id, p, w, has_w):
    n = pt.shape[0]
    d = 0.0
    for j in range(n):
        v = pt[j]
        ...
        if d_hi < 0.0:
            d_hi = -d_hi
        delta = d_lo if d_lo > d_hi else d_hi
        if metric_id == MET_CHEBYSHEV:
            if delta > d:
                d = delta
        elif metric_id == MET_MANHATTAN:
...
@njit(cache=True)
def _ball_max_rdist(bounds, radius, i_node, pt, metric_id, p, w, V, VI, has_w, has_V, has_VI):
    d = _dist(pt, bounds[0, i_node], metric_id, p, w, V, VI, has_w, has_V, has_VI) + radius[i_node]
    return _dist_to_rdist(d, metric_id, p)


@njit(cache=True)
def _max_rdist(tree_kind, bounds, radius, i_node, pt, metric_id, p, w, V, VI, has_w, has_V, has_VI):
    if tree_kind == TREE_KD:
        return _kd_max_rdist(bounds, i_node, pt, metric_id, p, w, has_w)
    return _ball_max_rdist(bounds, radius, i_node, pt, metric_id, p, w, V, VI, has_w, has_V, has_VI)

@njit(inline='always', cache=True)
def _node_skip_count(idx_start, idx_end, i_node, self_id, leaf_of):
    n = idx_end[i_node] - idx_start[i_node]
    if self_id < 0 or self_id >= leaf_of.shape[0]:
        return n
    cur = leaf_of[self_id]
    while True:
        if cur == i_node:
...
@njit(inline='always', cache=True)
def _classify_rdist(rd, skip_r, knn_r, metric_id, p, angular_mode):
    dout = _out_from_rdist(rd, metric_id, p, angular_mode)
    if dout < skip_r:
        return 0
    if dout <= knn_r:
        return 1
...
@njit(inline='always', cache=True)
def _scan_node_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, i_node,
                         dist, ind, n_found, skip_r, knn_r, skipped,
                         self_id, do_write, angular_mode):
    n_feat = data.shape[1]
    for i in range(idx_start[i_node], idx_end[i_node]):
        idx = idx_array[i]
        if idx == self_id:
            continue
        d = _kd_l2_rdist(X, i_pt, data, idx, n_feat)
        kind = _classify_rdist(d, skip_r, knn_r, MET_EUCLIDEAN, 2.0, angular_mode)
        if kind == 0:
            if do_write == 0:
                skipped[i_pt] += 1
            continue
...
@njit(inline='always', cache=True)
def _scan_node_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, i_node,
                         dist, ind, n_found, skip_r, knn_r, skipped,
                         self_id, do_write):
    n_feat = data.shape[1]
    for i in range(idx_start[i_node], idx_end[i_node]):
...
@njit(cache=True)
def _dfs_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                   dist, ind, n_found, root, stack_node, stack_lb,
                   skip_r, knn_r, skipped, self_id, do_write, angular_mode):
    n_feat = data.shape[1]
    mn, mx = _kd_l2_min_max_rdist(bounds, root, X, i_pt, n_feat)
    if skip_r > 0.0 and _out_from_rdist(mx, MET_EUCLIDEAN, 2.0, angular_mode) < skip_r:
        if do_write == 0:
            skipped[i_pt] += _node_skip_count(idx_start, idx_end, root, self_id, leaf_of)
        return
    if _out_from_rdist(mn, MET_EUCLIDEAN, 2.0, angular_mode) > knn_r:
        return
    stack_node[0] = root
    stack_lb[0] = mn
    sp = 1
    while sp > 0:
        sp -= 1
        i_node = stack_node[sp]
        if _out_from_rdist(stack_lb[sp], MET_EUCLIDEAN, 2.0, angular_mode) > knn_r:
            continue
        mn, mx = _kd_l2_min_max_rdist(bounds, i_node, X, i_pt, n_feat)
        if skip_r > 0.0 and _out_from_rdist(mx, MET_EUCLIDEAN, 2.0, angular_mode) < skip_r:
            if do_write == 0:
                skipped[i_pt] += _node_skip_count(idx_start, idx_end, i_node, self_id, leaf_of)
            continue
        if _out_from_rdist(mn, MET_EUCLIDEAN, 2.0, angular_mode) > knn_r:
            continue
        if _out_from_rdist(mx, MET_EUCLIDEAN, 2.0, angular_mode) <= knn_r or is_leaf[i_node]:
            _scan_node_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, i_node,
                                 dist, ind, n_found, skip_r, knn_r, skipped,
                                 self_id, do_write, angular_mode)
            continue
        i1 = 2 * i_node + 1
        ...
        if lb1 <= lb2:
            if _out_from_rdist(lb2, MET_EUCLIDEAN, 2.0, angular_mode) <= knn_r:
                stack_node[sp] = i2
                stack_lb[sp] = lb2
                sp += 1
            if _out_from_rdist(lb1, MET_EUCLIDEAN, 2.0, angular_mode) <= knn_r:
                stack_node[sp] = i1
                stack_lb[sp] = lb1
                sp += 1
        else:
...
@njit(cache=True)
def _dfs_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                   dist, ind, n_found, root, stack_node, stack_lb,
                   skip_r, knn_r, skipped, self_id, do_write):
    n_feat = data.shape[1]
    mn, mx = _kd_l1_min_max_rdist(bounds, root, X, i_pt, n_feat)
    if skip_r > 0.0 and mx < skip_r:
        if do_write == 0:
            skipped[i_pt] += _node_skip_count(idx_start, idx_end, root, self_id, leaf_of)
        return
    if mn > knn_r:
    ...
    while sp > 0:
        ...
        if mn > knn_r:
            continue
        if mx <= knn_r or is_leaf[i_node]:
            _scan_node_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, i_node,
                                 dist, ind, n_found, skip_r, knn_r, skipped,
                                 self_id, do_write)
            continue
        i1 = 2 * i_node + 1
...
@njit(cache=True)
def _query_one_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                         leaf_of, dist, ind, n_found, stack_node, stack_lb,
                         skip_r, knn_r, skipped, self_id, do_write, angular_mode):
    n_feat = data.shape[1]
    start = leaf_of[self_id] if 0 <= self_id < leaf_of.shape[0] else -1
    if start < 0:
        _dfs_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                       dist, ind, n_found, 0, stack_node, stack_lb,
                       skip_r, knn_r, skipped, self_id, do_write, angular_mode)
        return
    mn, mx = _kd_l2_min_max_rdist(bounds, start, X, i_pt, n_feat)
    if skip_r > 0.0 and _out_from_rdist(mx, MET_EUCLIDEAN, 2.0, angular_mode) < skip_r:
        if do_write == 0:
            skipped[i_pt] += _node_skip_count(idx_start, idx_end, start, self_id, leaf_of)
    elif _out_from_rdist(mn, MET_EUCLIDEAN, 2.0, angular_mode) <= knn_r:
        _scan_node_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, start,
                             dist, ind, n_found, skip_r, knn_r, skipped,
                             self_id, do_write, angular_mode)
    i_node = start
    while i_node != 0:
        ...
        _dfs_kd_l2_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                       dist, ind, n_found, sibling, stack_node, stack_lb,
                       skip_r, knn_r, skipped, self_id, do_write, angular_mode)
        i_node = (i_node - 1) // 2


@njit(cache=True)
def _query_one_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                         leaf_of, dist, ind, n_found, stack_node, stack_lb,
                         skip_r, knn_r, skipped, self_id, do_write):
    start = leaf_of[self_id] if 0 <= self_id < leaf_of.shape[0] else -1
    if start < 0:
        _dfs_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                       dist, ind, n_found, 0, stack_node, stack_lb,
                       skip_r, knn_r, skipped, self_id, do_write)
        return
    mn, mx = _kd_l1_min_max_rdist(bounds, start, X, i_pt, data.shape[1])
    if skip_r > 0.0 and mx < skip_r:
        if do_write == 0:
            skipped[i_pt] += _node_skip_count(idx_start, idx_end, start, self_id, leaf_of)
    elif mn <= knn_r:
        _scan_node_kd_l1_ext(X, i_pt, data, idx_array, idx_start, idx_end, start,
                             dist, ind, n_found, skip_r, knn_r, skipped,
                             self_id, do_write)
    i_node = start
    while i_node != 0:
...
@njit(cache=True)
def _dfs_generic_ext(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                     tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                     dist, ind, n_found, leaf_of, root, stack_node, stack_lb,
                     skip_r, knn_r, skipped, self_id, do_write, angular_mode):
    if skip_r > 0.0:
        mx = _max_rdist(tree_kind, bounds, radius, root, pt, metric_id, p,
                        w, V, VI, has_w, has_V, has_VI)
        if _out_from_rdist(mx, metric_id, p, angular_mode) < skip_r:
            if do_write == 0:
                skipped[i_pt] += _node_skip_count(idx_start, idx_end, root, self_id, leaf_of)
            return
    lb = _min_rdist(tree_kind, bounds, radius, root, pt, metric_id, p,
                    w, V, VI, has_w, has_V, has_VI)
    if _out_from_rdist(lb, metric_id, p, angular_mode) > knn_r:
        return
    stack_node[0] = root
    stack_lb[0] = lb
    sp = 1
    while sp > 0:
        sp -= 1
        i_node = stack_node[sp]
        if _out_from_rdist(stack_lb[sp], metric_id, p, angular_mode) > knn_r:
            continue
        if skip_r > 0.0:
            mx = _max_rdist(tree_kind, bounds, radius, i_node, pt, metric_id, p,
                            w, V, VI, has_w, has_V, has_VI)
            if _out_from_rdist(mx, metric_id, p, angular_mode) < skip_r:
                if do_write == 0:
                    skipped[i_pt] += _node_skip_count(idx_start, idx_end, i_node, self_id, leaf_of)
        ...
        if is_leaf[i_node]:
            for i in range(idx_start[i_node], idx_end[i_node]):
                ...
                if idx == self_id:
                    continue
                d = _rdist(pt, data[idx], metric_id, p, w, V, VI, has_w, has_V, has_VI)
                kind = _classify_rdist(d, skip_r, knn_r, metric_id, p, angular_mode)
                if kind == 0:
                    if do_write == 0:
                        skipped[i_pt] += 1
                    continue
        ...
        lb2 = _min_rdist(tree_kind, bounds, radius, i2, pt, metric_id, p,
                         w, V, VI, has_w, has_V, has_VI)
        if lb1 <= lb2:
            if _out_from_rdist(lb2, metric_id, p, angular_mode) <= knn_r:
                stack_node[sp] = i2
                stack_lb[sp] = lb2
                sp += 1
            if _out_from_rdist(lb1, metric_id, p, angular_mode) <= knn_r:
                stack_node[sp] = i1
                stack_lb[sp] = lb1
                sp += 1
        else:
...
@njit(cache=True)
def _query_one_ext(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                   tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                   dist, ind, n_found, leaf_of, stack_node, stack_lb,
                   skip_r, knn_r, skipped, self_id, do_write, angular_mode):
    n_feat = data.shape[1]
    pt = np.empty(n_feat, dtype=np.float64)
    for j in range(n_feat):
    ...
    start = leaf_of[self_id] if 0 <= self_id < leaf_of.shape[0] else -1
    if start < 0:
        _dfs_generic_ext(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                         tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                         dist, ind, n_found, leaf_of, 0, stack_node, stack_lb,
                         skip_r, knn_r, skipped, self_id, do_write, angular_mode)
        return
    if skip_r > 0.0:
        mx = _max_rdist(tree_kind, bounds, radius, start, pt, metric_id, p,
                        w, V, VI, has_w, has_V, has_VI)
        if _out_from_rdist(mx, metric_id, p, angular_mode) < skip_r:
            if do_write == 0:
                skipped[i_pt] += _node_skip_count(idx_start, idx_end, start, self_id, leaf_of)
        else:
            lb = _min_rdist(tree_kind, bounds, radius, start, pt, metric_id, p,
                            w, V, VI, has_w, has_V, has_VI)
            if _out_from_rdist(lb, metric_id, p, angular_mode) <= knn_r:
    ...
    while i_node != 0:
        ...
        _dfs_generic_ext(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                         tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                         dist, ind, n_found, leaf_of, sibling, stack_node, stack_lb,
                         skip_r, knn_r, skipped, self_id, do_write, angular_mode)
        i_node = (i_node - 1) // 2


@njit(parallel=True, cache=True)
def _query_all_kd_l2_ext(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, n_found, skipped, skip_r, knn_r,
                         query_ids, do_write, do_finish, angular_mode):
    n_queries = X.shape[0]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_kd_l2_ext(X, i, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                             leaf_of, dist, ind, n_found, stack_node[i], stack_lb[i],
                             skip_r[i], knn_r[i], skipped, query_ids[i], do_write, angular_mode)
        if do_finish != 0:
            n_use = n_found[i]
            if n_use > 1:
                _sort_row(dist, ind, i, n_use)
            for j in range(n_use):
                dist[i, j] = math.sqrt(dist[i, j])


@njit(cache=True)
def _query_all_kd_l2_ext_seq(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                             dist, ind, n_found, skipped, skip_r, knn_r,
                             query_ids, do_write, do_finish, angular_mode):
    n_queries = X.shape[0]
    stack_node = np.empty((_STACK_CAP,), dtype=np.intp)
...
@njit(parallel=True, cache=True)
def _query_all_kd_l1_ext(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, n_found, skipped, skip_r, knn_r,
                         query_ids, do_write, do_finish):
    n_queries = X.shape[0]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_kd_l1_ext(X, i, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                             leaf_of, dist, ind, n_found, stack_node[i], stack_lb[i],
                             skip_r[i], knn_r[i], skipped, query_ids[i], do_write)
        if do_finish != 0:
            n_use = n_found[i]
            if n_use > 1:
                _sort_row(dist, ind, i, n_use)
...
@njit(cache=True)
def _query_all_kd_l1_ext_seq(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                             dist, ind, n_found, skipped, skip_r, knn_r,
                             query_ids, do_write, do_finish):
    n_queries = X.shape[0]
    stack_node = np.empty((_STACK_CAP,), dtype=np.intp)
...
@njit(parallel=True, cache=True)
def _query_all_ext(X, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                   tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                   leaf_of, dist, ind, n_found, skipped, skip_r, knn_r,
                   query_ids, do_write, do_finish, angular_mode):
    n_queries = X.shape[0]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_ext(X, i, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                       tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                       dist, ind, n_found, leaf_of, stack_node[i], stack_lb[i],
                       skip_r[i], knn_r[i], skipped, query_ids[i], do_write, angular_mode)
        if do_finish != 0:
            n_use = n_found[i]
            if n_use > 1:
...
@njit(cache=True)
def _query_all_ext_seq(X, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                       tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                       leaf_of, dist, ind, n_found, skipped, skip_r, knn_r,
                       query_ids, do_write, do_finish, angular_mode):
    n_queries = X.shape[0]
...
@njit(inline='always', cache=True)
def _scan_node_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end, i_node, dist, ind,
                     tau, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    for i in range(idx_start[i_node], idx_end[i_node]):
        idx = idx_array[i]
        if idx == self_id:
...
@njit(inline='always', cache=True)
def _scan_node_kd_l1(X, i_pt, data, idx_array, idx_start, idx_end, i_node, dist, ind,
                     tau, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    for i in range(idx_start[i_node], idx_end[i_node]):
        idx = idx_array[i]
...
@njit(cache=True)
def _dfs_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
               dist, ind, tau, root, stack_node, stack_lb, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    mn, mx = _kd_l2_min_max_rdist(bounds, root, X, i_pt, n_feat)
    if skip_rdist > 0.0 and mx < skip_rdist:
        skipped[i_pt] += _node_skip_count(idx_start, idx_end, root, self_id, leaf_of)
        return tau
    if mn > tau:
        return tau
    ...
    while sp > 0:
        ...
        if mx <= tau or is_leaf[i_node]:
            tau = _scan_node_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end,
                                   i_node, dist, ind, tau, skip_rdist, skipped, self_id)
            continue
        i1 = 2 * i_node + 1
        i2 = i1 + 1
        lb1, _ = _kd_l2_min_max_rdist(bounds, i1, X, i_pt, n_feat)
...
@njit(cache=True)
def _dfs_kd_l1(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
               dist, ind, tau, root, stack_node, stack_lb, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    mn, mx = _kd_l1_min_max_rdist(bounds, root, X, i_pt, n_feat)
    if skip_rdist > 0.0 and mx < skip_rdist:
...
@njit(cache=True)
def _query_one_kd_l2_skip(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                     leaf_of, dist, ind, stack_node, stack_lb, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    tau = dist[i_pt, 0]
    start = leaf_of[self_id] if 0 <= self_id < leaf_of.shape[0] else -1
    if start < 0:
        return _dfs_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                          dist, ind, tau, 0, stack_node, stack_lb, skip_rdist, skipped, self_id)
    mn, mx = _kd_l2_min_max_rdist(bounds, start, X, i_pt, n_feat)
    if skip_rdist > 0.0 and mx < skip_rdist:
        skipped[i_pt] += _node_skip_count(idx_start, idx_end, start, self_id, leaf_of)
    else:
        tau = _scan_node_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end,
                               start, dist, ind, tau, skip_rdist, skipped, self_id)
    i_node = start
    while i_node != 0:
        sibling = i_node + 1 if i_node % 2 == 1 else i_node - 1
        tau = _dfs_kd_l2(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, tau, sibling, stack_node, stack_lb, skip_rdist, skipped, self_id)
        i_node = (i_node - 1) // 2
    return tau


@njit(cache=True)
def _query_one_kd_l1_skip(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                     leaf_of, dist, ind, stack_node, stack_lb, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    tau = dist[i_pt, 0]
    start = leaf_of[self_id] if 0 <= self_id < leaf_of.shape[0] else -1
    if start < 0:
        return _dfs_kd_l1(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                          dist, ind, tau, 0, stack_node, stack_lb, skip_rdist, skipped, self_id)
    mn, mx = _kd_l1_min_max_rdist(bounds, start, X, i_pt, n_feat)
    if skip_rdist > 0.0 and mx < skip_rdist:
        skipped[i_pt] += _node_skip_count(idx_start, idx_end, start, self_id, leaf_of)
    else:
        tau = _scan_node_kd_l1(X, i_pt, data, idx_array, idx_start, idx_end,
    ...
    while i_node != 0:
        sibling = i_node + 1 if i_node % 2 == 1 else i_node - 1
        tau = _dfs_kd_l1(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, tau, sibling, stack_node, stack_lb, skip_rdist, skipped, self_id)
        i_node = (i_node - 1) // 2
    return tau
...
def _dfs_generic(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                 tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind, leaf_of,
                 tau, root, stack_node, stack_lb, skip_rdist, skipped, self_id):
    if skip_rdist > 0.0:
        mx = _max_rdist(tree_kind, bounds, radius, root, pt, metric_id, p,
                        w, V, VI, has_w, has_V, has_VI)
        if mx < skip_rdist:
...
@njit(cache=True)
def _query_one_skip(X, i_pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
               tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind,
               leaf_of, stack_node, stack_lb, skip_rdist, skipped, self_id):
    n_feat = data.shape[1]
    pt = np.empty(n_feat, dtype=np.float64)
    for j in range(n_feat):
        pt[j] = X[i_pt, j]
    ...
    if start < 0:
        return _dfs_generic(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                            tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind, leaf_of,
                            tau, 0, stack_node, stack_lb, skip_rdist, skipped, self_id)
    if skip_rdist > 0.0:
        mx = _max_rdist(tree_kind, bounds, radius, start, pt, metric_id, p,
                        w, V, VI, has_w, has_V, has_VI)
        if mx < skip_rdist:
    ...
    while i_node != 0:
        ...
        tau = _dfs_generic(X, i_pt, pt, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                           tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind, leaf_of,
                           tau, sibling, stack_node, stack_lb, skip_rdist, skipped, self_id)
        i_node = (i_node - 1) // 2
    return tau


@njit(parallel=True, cache=True)
def _query_all_kd_l2_skip(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                     dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_kd_l2_skip(X, i, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                         leaf_of, dist, ind, stack_node[i], stack_lb[i],
                         skip_rdist[i], skipped, query_ids[i])
        if do_sort:
            _sort_row(dist, ind, i, k_nbrs)
        for j in range(k_nbrs):
            dist[i, j] = math.sqrt(dist[i, j])
...
@njit(cache=True)
def _query_all_kd_l2_skip_seq(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    stack_node = np.empty((_STACK_CAP,), dtype=np.intp)
...
@njit(parallel=True, cache=True)
def _query_all_kd_l1_skip(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                     dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_kd_l1_skip(X, i, data, idx_array, idx_start, idx_end, is_leaf, bounds,
                         leaf_of, dist, ind, stack_node[i], stack_lb[i],
                         skip_rdist[i], skipped, query_ids[i])
        if do_sort:
            _sort_row(dist, ind, i, k_nbrs)
...
@njit(cache=True)
def _query_all_kd_l1_skip_seq(X, data, idx_array, idx_start, idx_end, is_leaf, bounds, leaf_of,
                         dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    stack_node = np.empty((_STACK_CAP,), dtype=np.intp)
...
@njit(parallel=True, cache=True)
def _query_all_skip(X, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
               tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
               leaf_of, dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    stack_node = np.empty((n_queries, _STACK_CAP), dtype=np.intp)
    stack_lb = np.empty((n_queries, _STACK_CAP), dtype=np.float64)
    for i in prange(n_queries):
        _query_one_skip(X, i, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                   tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind,
                   leaf_of, stack_node[i], stack_lb[i], skip_rdist[i], skipped, query_ids[i])
        if do_sort:
            _sort_row(dist, ind, i, k_nbrs)
        for j in range(k_nbrs):
            dist[i, j] = _rdist_to_dist(dist[i, j], metric_id, p)
...
@njit(cache=True)
def _query_all_skip_seq(X, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                   tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI,
                   leaf_of, dist, ind, skipped, skip_rdist, do_sort, query_ids):
    n_queries = X.shape[0]
    k_nbrs = dist.shape[1]
    ...
    for i in range(n_queries):
        _query_one_skip(X, i, data, idx_array, idx_start, idx_end, is_leaf, radius, bounds,
                   tree_kind, metric_id, p, w, V, VI, has_w, has_V, has_VI, dist, ind,
                   leaf_of, stack_node, stack_lb, skip_rdist[i], skipped, query_ids[i])
        if do_sort:
            _sort_row(dist, ind, i, k_nbrs)
        for j in range(k_nbrs):
            dist[i, j] = _rdist_to_dist(dist[i, j], metric_id, p)
...
class NeighborTree:
    ...
    def query_init(self, k=1, return_distance=True, dualtree=False, breadth_first=False,
              sort_results=True):
        """Return k nearest neighbors for every training point, excluding itself.
    ...
    def _run_skip_knn(self, X, dist_arr, ind_arr, knn_skip, skip_rdist, query_ids, parallel):
        do_sort = 1
        args = (
            X, self._tree_data, self._idx_array, self._idx_start, self._idx_end,
            self._is_leaf, self._bounds, self._leaf_of, dist_arr, ind_arr,
            knn_skip, skip_rdist, do_sort, query_ids,
        )
        if self.tree_kind == TREE_KD and self.metric_id == MET_EUCLIDEAN and not self.has_weight:
            (_query_all_kd_l2_skip if parallel else _query_all_kd_l2_skip_seq)(*args)
        elif self.tree_kind == TREE_KD and self.metric_id == MET_MANHATTAN and not self.has_weight:
            (_query_all_kd_l1_skip if parallel else _query_all_kd_l1_skip_seq)(*args)
        else:
            generic = (
                X, self._tree_data, self._idx_array, self._idx_start, self._idx_end,
                self._is_leaf, self._radius, self._bounds, self.tree_kind, self.metric_id,
                self.p, self._weight, self._V, self._VI, self.has_weight, self.has_V,
                self.has_VI, self._leaf_of, dist_arr, ind_arr, knn_skip, skip_rdist, do_sort,
                query_ids,
            )
            (_query_all_skip if parallel else _query_all_skip_seq)(*generic)
    ...
    def _run_extend(self, X, dist_arr, ind_arr, n_found, knn_skip, skip_r, knn_r,
                    query_ids, do_write, do_finish, parallel):
        if self.tree_kind == TREE_KD and self.metric_id == MET_EUCLIDEAN and not self.has_weight:
            args = (
                X, self._tree_data, self._idx_array, self._idx_start, self._idx_end,
                self._is_leaf, self._bounds, self._leaf_of, dist_arr, ind_arr,
                n_found, knn_skip, skip_r, knn_r, query_ids, do_write, do_finish,
                self.angular_mode,
            )
            (_query_all_kd_l2_ext if parallel else _query_all_kd_l2_ext_seq)(*args)
        elif self.tree_kind == TREE_KD and self.metric_id == MET_MANHATTAN and not self.has_weight:
            args = (
                X, self._tree_data, self._idx_array, self._idx_start, self._idx_end,
                self._is_leaf, self._bounds, self._leaf_of, dist_arr, ind_arr,
            ...
            (_query_all_kd_l1_ext if parallel else _query_all_kd_l1_ext_seq)(*args)
        else:
            generic = (
                X, self._tree_data, self._idx_array, self._idx_start, self._idx_end,
                self._is_leaf, self._radius, self._bounds, self.tree_kind, self.metric_id,
                self.p, self._weight, self._V, self._VI, self.has_weight, self.has_V,
                self.has_VI, self._leaf_of, dist_arr, ind_arr, n_found, knn_skip,
                skip_r, knn_r, query_ids, do_write, do_finish, self.angular_mode,
            )
            (_query_all_ext if parallel else _query_all_ext_seq)(*generic)

    def query_extend(self, indices, skip_radii, knn_scope):
        """Neighbors beyond the skip ball, at least ``knn_scope`` per index.

        Parameters
        ----------
        indices : array of int
            Training-set indexes to query. Result rows follow this order.
        skip_radii : array of shape (n_samples,)
            Open inner ball in the original metric. Neighbors with
            ``d < skip_radii[i]`` are counted in ``knn_skip`` and omitted.
        knn_scope : int or array
            Per-index neighbor count beyond the skip ball (replaces a communal
            scalar K). Length ``n_samples`` (indexed by training id) or one
            value per queried index. The k-th neighbor's distance is a closed
            outer bound, so ties at that distance are kept and the row can
            grow past ``knn_scope[i]``. Unused slots are ``0``.

        Returns
        -------
        knn_skip : ndarray of shape (n_queries,)
            ``|{p ≠ i : d(i, p) < skip_radii[i]}|`` for each queried index.
        dist : ndarray of shape (n_queries, width)
            ``width = max(max knn_scope, max row length)``. Trailing unused
            entries are 0.
        ind : ndarray of shape (n_queries, width)
        """
        ...
        if skip_arr.shape[0] != self.n_samples:
        ...
        if scope_in.ndim == 0:
            scope = np.full(self.n_samples, int(scope_in), dtype=np.intp)
        else:
            scope = np.ascontiguousarray(scope_in.reshape(-1))

        if n_queries == 0:
            width = 1 if scope.size == 0 else max(int(scope.max()), 1)
            return (np.empty(0, dtype=np.intp),
                    np.empty((0, width), dtype=np.float64),
                    np.empty((0, width), dtype=np.intp))

        if scope.shape[0] == self.n_samples:
            k_q = np.ascontiguousarray(scope[query_ids])
        elif scope.shape[0] == n_queries:
            k_q = scope
        else:
        ...
        if int(k_q.max()) > self.n_samples - 1:
            raise ValueError('knn_scope values must be less than the number of training points')

        max_k = int(k_q.max())
        skip_q = np.ascontiguousarray(skip_arr[query_ids])
        X = np.ascontiguousarray(self._tree_data[query_ids])
        parallel = n_queries >= 128

        skip_rdist = _as_skip_rdist(
            skip_arr, query_ids, self.angular_mode, self.metric_id, self.p)
        dist_h = np.full((n_queries, max_k), INF, dtype=np.float64)
        ind_h = np.zeros((n_queries, max_k), dtype=np.intp)
        knn_skip = np.zeros(n_queries, dtype=np.intp)
        self._run_skip_knn(X, dist_h, ind_h, knn_skip, skip_rdist, query_ids, parallel)

        knn_q = np.ascontiguousarray(dist_h[np.arange(n_queries), k_q - 1])
        dummy = np.zeros((n_queries, 1), dtype=np.float64)
        dummy_ind = np.zeros((n_queries, 1), dtype=np.intp)
        n_found = np.zeros(n_queries, dtype=np.intp)
        knn_skip[:] = 0
        self._run_extend(
            X, dummy, dummy_ind, n_found, knn_skip, skip_q, knn_q,
            query_ids, 0, 0, parallel)

        width = max(max_k, int(n_found.max()) if n_found.size else 0)
        dist_arr = np.zeros((n_queries, width), dtype=np.float64)
        ind_arr = np.zeros((n_queries, width), dtype=np.intp)
        if int(n_found.max()) > 0:
            n_found[:] = 0
            self._run_extend(
                X, dist_arr, ind_arr, n_found, knn_skip, skip_q, knn_q,
                query_ids, 1, 1, parallel)
