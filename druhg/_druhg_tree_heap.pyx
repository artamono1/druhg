...
# Typed min-heaps for MST targeting bulks: (float, int) for reciprocity,
# (int, int) for pending-edge counts.
# Author: AI generated
...
ctypedef fused heap_key_t:
    np.double_t
    np.intp_t


cdef inline bint _heap_less(heap_key_t ka, np.intp_t va,
                            heap_key_t kb, np.intp_t vb) nogil:
    # Same order as Python heapq on (key, val) tuples.
    return ka < kb or (ka == kb and va < vb)


cdef inline void _heap_siftdown(heap_key_t *keys, np.intp_t *vals,
                                np.intp_t startpos, np.intp_t pos) noexcept nogil:
    # Bubble newitem at pos up toward startpos (Python heapq._siftdown).
    cdef np.intp_t parentpos, new_val, parent_val
    cdef heap_key_t new_key, parent_key
    ...
    while pos > startpos:
        ...
        if not _heap_less(new_key, new_val, parent_key, parent_val):
            break
        keys[pos] = parent_key
        vals[pos] = parent_val
        pos = parentpos
...
cdef inline void _heap_siftup(heap_key_t *keys, np.intp_t *vals,
                              np.intp_t endpos, np.intp_t pos) noexcept nogil:
    # Bubble hole at pos down, then siftdown (Python heapq._siftup).
    cdef np.intp_t startpos, childpos, rightpos, new_val, child_val, right_val
    cdef heap_key_t new_key, child_key, right_key
    ...
    while childpos < endpos:
        ...
        if rightpos < endpos:
            ...
            if not _heap_less(child_key, child_val, right_key, right_val):
                childpos = rightpos
                child_key = right_key
                child_val = right_val
        keys[pos] = child_key
    ...
    keys[pos] = new_key
    vals[pos] = new_val
    _heap_siftdown(keys, vals, startpos, pos)
...
cdef class FloatIntMinHeap:
    ...
    cdef void _siftdown(self, np.intp_t startpos, np.intp_t pos) noexcept nogil:
        _heap_siftdown(self.keys, self.vals, startpos, pos)

    cdef void _siftup(self, np.intp_t pos) noexcept nogil:
        _heap_siftup(self.keys, self.vals, self.size, pos)
...
cdef class IntIntMinHeap:
    """Typed binary min-heap of (int key, int payload) pairs."""
    ...
    def __cinit__(self, np.intp_t capacity=8):
        ...
        self.keys = NULL
        self.vals = NULL
        self.keys = <np.intp_t *> PyMem_Malloc(capacity * sizeof(np.intp_t))
        self.vals = <np.intp_t *> PyMem_Malloc(capacity * sizeof(np.intp_t))
        if self.keys == NULL or self.vals == NULL:
            raise MemoryError()
        self.capacity = capacity
    ...
    cdef void _grow(self, np.intp_t need) except *:
        cdef np.intp_t new_cap
        cdef np.intp_t *new_keys
        cdef np.intp_t *new_vals

        new_cap = self.capacity << 1
        if new_cap < need:
            new_cap = need
        new_keys = <np.intp_t *> PyMem_Realloc(self.keys, new_cap * sizeof(np.intp_t))
        if new_keys == NULL:
            raise MemoryError()
        self.keys = new_keys
        new_vals = <np.intp_t *> PyMem_Realloc(self.vals, new_cap * sizeof(np.intp_t))
