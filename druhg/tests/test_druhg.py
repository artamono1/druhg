...
def test_unionfind_to_linkage_synthetic():
    ...
    from druhg import unionfind_to_linkage
...
def test_hierarchy_linkage():
    from scipy.cluster.hierarchy import is_valid_linkage, fcluster, to_tree
    ...
    assert Z[-1, 3] == len(XX)
    assert np.all(Z[:, 2] >= 0)
    # SciPy consumers accept the matrix.
...
def test_labels_to_link_color_func():
    ...
    from druhg import labels_to_link_color_func
